   ...
   public class MostEventfulWeekCalculatorTest
    {
        ...
        public void FindInTwoTrackerWithFactMostEventfulWeek_CalculateSuccess()
        {
            ...
            var tracker1 = CreateTracker(userId, "Покупка");
            var tracker2 = CreateTracker(userId, "Продажа");
            const int year = 2020;
            var eventsOfTracker1 = CreateEventDuringYear(tracker1.Id, userId, 100, year);
            var eventsOfTracker2 = CreateEventDuringYear(tracker2.Id, userId, 100, year);
        ...
        public void TrackerHaveZeroEvent_CalculateFailure()
        {
            //arrange
            var userId = Guid.NewGuid();
            var tracker = CreateTracker(userId, "Покупка");
            const int numFailureEvents = 0;
            var eventsTracker = CreateEventDuringYear(tracker.Id, userId, numFailureEvents, 2020);
            _eventRepository.AddRangeOfEvents(eventsTracker);
            var allEvents = _eventRepository.LoadAllTrackerEvents(tracker.Id);
            var trackerWithItsEvents = new List<TrackerWithItsEvents>
            {
                new TrackerWithItsEvents(tracker, allEvents)
            };
        ...
        private static IList<Event> CreateEventDuringYear(Guid trackerId, Guid userId, int num, int year)
        {
            var events = new List<Event>();
            for (var i = 0; i < num; i++)
            {
                var newEvent = CreateEventWithFixTime( trackerId, userId, RandomDayDuringYear(year));
                events.Add(newEvent);
            }
            return events;
        }
