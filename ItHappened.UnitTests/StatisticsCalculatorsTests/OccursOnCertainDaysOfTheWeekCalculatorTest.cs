...
namespace ItHappend.UnitTests.StatisticsCalculatorsTests
{
    public class OccursOnCertainDaysOfTheWeekCalculatorTest
    {
        ...
        public void EventTrackerHasTwoRatingAndEvents_CalculateSuccess()
        {
            ...
            //act 
            var fact = new OccursOnCertainDaysOfTheWeekCalculator().Calculate(eventTracker);
        ...
        private static List<Event> CreateEvents(int num)
        {
            var dateList = Enumerable.Repeat(DateTimeOffset.UtcNow, num).ToList();
            return dateList
                .Select((t, i) =>
                    EventBuilder
                        .Event(Guid.NewGuid(), Guid.NewGuid(), t, $"Event_{i}")
                        .Build())
                .ToList();
        }
    }
